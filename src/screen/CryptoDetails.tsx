...
const CryptoDetails: React.FC = () => {
  ...
  return (
    <div className="crypto__details h-fit w-full flex flex-col justify-center items-center gap-8">
      <div className=" text-3xl flex justify-start w-full font-semibold text-gray-600">
        Realtime Updates
      </div>
      <div className="h-1/4 w-3/4 bg-slate-200 rounded-md p-4 flex flex-col justify-between gap-6">
        <div className="flex flex-wrap items-center justify-between max-md:flex-col max-md:items-center max-md:gap-4">
          ...
          <div className="price-percentahe flex items-center gap-2 ">
            <span
              className={`text-xl font-semibold text-${color}-700 border-r-2 pr-2 border-slate-700 `}
            >
            ...
            >
              ...
              {Number(selected?.changePercent24Hr) > 0 ? (
                <SquareChevronUp size={15} color="#15803d" />
              ) : (
                <SquareChevronDown size={15} color="#b91c1c" />
              )}
            </span>
          </div>
        </div>
