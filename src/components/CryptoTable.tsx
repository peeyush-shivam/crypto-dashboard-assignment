...
const CryptoTable: React.FC = () => {
  ...
  const imageBodyTemplate = (currency: Currency) => {
    return (
      <div
        className="flex items-center justify-between min-w-8 gap-2 relative pt-2 pb-2 cursor-default"
    ...
    return (
      <div>
        <Link to={`/crypto/${currency.id}`} state={{ currency }}>
          <span className=" cursor-pointer bg-[#cb9140] rounded-sm p-2">
            {currency.name}
          </span>
        </Link>
      </div>
  ...
  const symbolBodyTemplate = (currency: Currency) => {
    return <div className=" cursor-default">{currency.symbol}</div>;
  };
